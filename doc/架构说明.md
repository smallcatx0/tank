...
> **一句话定位**：一个结构规范、分层清晰、可快速落地的 Go HTTP 服务端脚手架，内置统一响应/异常体系、链路追踪、分布式任务消费、配置热加载与优雅退出等生产级能力。
...
同一套代码产出两个可执行体，共享 `bootstrap / models / pkg` 等基础设施：

| 进程 | 入口 | 职责 |
| --- | --- | --- |
| API 服务 | `cmd/main.go` | 对外提供 HTTP 接口（用户端 `/v1`、后台 `/admin`、探活） |
| 任务 Worker | `cmd/sth_worker/main.go` | 常驻消费异步任务（消息队列 / 数据库表任务） |
...
1. **清晰的职责分层**：接口层不掺业务、业务层不碰 HTTP，团队协作与测试友好。
2. **统一响应与异常体系**：错误码规范集中，安全与调试兼顾。
3. **入参校验规范化**：两段式校验，规则与错误返回统一。
4. **双模式分布式任务框架**：高实时（MQ）与可持久化（DB 表）按需选型，仅依赖 MySQL+Redis。
5. **配置驱动的定时策略**：清理/重试规则入库，秒级 cron + 分布式锁。
6. **可观测性内建**：traceid 全链路贯穿、结构化 JSON 日志、SQL 慢查询接管、系统指标心跳、可选 SLS 投递。
7. **运维友好**：`/reload` 配置热加载、`/healthz` `/ready` 探活、连接池与日志级别集中配置。
8. **可复用组件库**：`pkg` 内组件已解耦，可跨项目直接引入。
...
- 各包使用说明：`middleware/resp/readme.md`、`models/valid/readme.md`、`pkg/sth_job/readme.md`、`pkg/glog/readme.md`
- 设计文档：`doc/规范.md`、`doc/接口文档.md`、`doc/分布式任务消费.md`
- 数据库结构：`doc/mysql.sql`
